...
% Modified by Alan Stone on 02/01/2019
% reads Bruker fid file and extracts single channel (channelID) from coil with
% more than one channel (nchannels)

function [iField voxel_size matrix_size TE delta_TE CF Affine3D B0_dir TR NumEcho] = Read_Bruker_raw_singlecoil(fid_dir,channelID,nchannels,oversampling_zdir)
...
xsize = matrix_size(1);
ysize = matrix_size(2);
if oversampling_zdir
    matrix_size(3) = matrix_size(3)*2;
end
zsize = matrix_size(3);
echo_no = NumEcho;
...
%for k = 1:length(acq_no)
    ...
    data = fread(fid,Inf,'int32');
     fclose(fid);
    % NOTE: Commented this out (Alan Stone)
    % if xsize<=128
    %     xres = 128;
    % elseif xsize<=256
    %     xres = 256;
    ...
    % NOTE end: (AlanStone)
    % AlanStone: start
    data=reshape(data,[2 xres nchannels echo_no ysize zsize ]);
    % NOTE: Alan Stone ... Removing 2nd channel for the time being
    data = squeeze(data(:,:,channelID,:,:,:));
    % AlanStone: end
    data=data(:,1:xsize,:,:,:);
    ...
    if echo_no >1
        for k=1 : echo_no
            % TODO: This isn't working phase maps
            %       Likley because of phase offset correction below
            % AlanStone: start
            % if mod(k+1,2)
            %     data0(:,k,:,:) = flip(data0(:,k,:,:), 1);
            % end
            % AlanStone: end
    ...
    for i = 1 : echo_no
        ...
        % AlanStone: start
        % if mod(k+1,2)
        %     iField(:,:,:,i) = ifftn(ifftshift(fftshift(fftn(squeeze(iField(:,:,:,i)))).*flip(phasor,1)));
        % else
