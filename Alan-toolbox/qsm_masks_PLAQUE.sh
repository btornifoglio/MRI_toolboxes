...
# INPUTS:
# 		dirname: masks directory produced by qsm_analsyis.m has run in "mask_mode"
# 		thr_tube: threshold chosen to extract tube and contents from nackground in magnitude combined image
# 		bot_slice: bottom slice of mask (indexing begins at 0)
# 		top_slice: top slice of mask (indexing begins at 0)
# 		thr_vessels_bubbles: threshold chosen to remove vessels and bubbles in R2s image
# 		thr_vessels: threshold chosen to localise to vessels
# 		sigma: smoothing kernal - sigma = FWHM/2.3548. FWHM in mm

if [ "$#" -ne 7 ]; then
	echo '     ===> usage: qsm_masks_PLAQUE.sh dirname thr_tube bot_slice top_slice thr_vessels_bubbles thr_vessels sigma'
	echo '     ===> e.g. qsm_make_PLAQUE.sh 20190703_AS1/qsm_masks 0.15 64 126 30 10 0.042'
	exit
fi
...
sigma=$7 # smoothing kernal - sigma = FWHM/2.3548. FWHM in mm
...
## BUBBLES MASK ##
##################
# ... works well for plaques with bubbles and calcifications
...
## KEEP VESSELS & PBS ##
########################
# ... remove bubbles and background MASK
